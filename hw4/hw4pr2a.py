...
def grad_logreg(X, y, W, reg=0.0):
	...
	"*** END YOUR CODE HERE ***"
	temp_val = y - sigmoid(np.matmul(X,W))
	return reg * W - np.matmul(X.T, temp_val)
...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	vals = sigmoid(np.matmul(X, W))
	temp = np.multiply(y, np.log(vals)) + np.multiply(1 - y, np.log(1 - vals))
	nll = -sum(temp) + 0.5 * reg * (np.linalg.norm(W) ** 2)
	return -nll
...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		if (iter_num + 1) % print_freq == 0:
			print(f'-- Iteration {iter_num + 1} - negative log likelihood {round(nll[0],2)}')
...
def newton_step(X, y, W, reg=0.0):
	...
	diag = np.diag(np.squeeze(np.asarray(np.multiply(sigmoid(np.matmul(X, W)), 1 - sigmoid(np.matmul(X, W))))))
	H = np.matmul(X.T,np.matmul(diag, X)) + reg * np.eye(X.shape[1])
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	for index in range(len(reg_list)):
		reg = reg_list[index]
		W_opt, obj = grad_descent(X_train, y_train, reg = reg, lr = 1e-3, print_freq = 100)
...
if __name__ == '__main__':
	...
	W_gd, nll_list_gd = grad_descent(X_train_logreg, y_train_logreg, reg = 1e-3)
