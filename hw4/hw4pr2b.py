...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	temp_1 = np.exp(np.matmul(X,W))
	mu = temp_1 / temp_1.sum(axis=1).reshape(-1,1)
	term_2 = reg * np.trace(np.matmul(W.T, W))
	NLL = -(y * np.log(mu)).sum(axis=1).sum() + term_2
	"*** END YOUR CODE HERE ***"
	return NLL
...
def grad_softmax(X, y, W, reg=0.0):
	...
	temp_1 = np.exp(np.matmul(X,W))
	mu = temp_1 / temp_1.sum(axis=1).reshape(-1,1)
	grad_nll =  np.matmul(X.T, (mu - y)) + reg * W
	return grad_nll
...
def predict(X, W):
	...
	mu = np.matmul(X, W)
	e_mu = np.exp(mu)
	return np.argmax(e_mu / e_mu.sum(axis=1).reshape(-1, 1), axis=1).reshape(-1, 1)
