...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		"*** YOUR CODE HERE ***"
		for i in range(m):
			label[i] = np.argsort(np.linalg.norm(X[i, :] - clusters, axis=1)).item(0)
		# update centers
		for groupNumber in range(k):
			indexCenter = np.where(label == groupNumber)[0]
			if len(indexCenter) > 0:
				clusters[groupNumber, :] = X[indexCenter].mean(axis=0)
...
if __name__ == '__main__':
	...
	"*** YOUR CODE HERE ***"
	plt.plot(range(1, 21), cost_k_list, 'rx')
	plt.plot(opt_k, min(cost_k_list), 'bo')
	"*** END YOUR CODE HERE ***"
	...
	"*** YOUR CODE HERE ***"
	clustersBest, labelBest, cost_list = k_means(X, opt_k)
	centersFromClusters = clusters[label.flatten()]
	data_plot, = plt.plot(X[:, 0], X[:, 1], 'rx')
	center_plot, = plt.plot(centersFromClusters[:, 0], centersFromClusters[:, 1], 'bo')
