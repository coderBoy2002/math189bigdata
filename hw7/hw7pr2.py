...
def find_cost(X, y, W , reg):
	...
	cost = float(np.matmul(error.T, error))+reg * np.abs(W).sum()
	cost = cost / len(y)
...
def find_grad(X, y, W, reg=0.0):
	...
	error_matrix = np.matmul(X, W) - y
	grad =  np.matmul(X.T, error_matrix) / X.shape[0]
	"*** END YOUR CODE HERE ***"
	return grad
...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		indecies = np.random.randint(0,m,size=batch_size)
		W_grad = find_grad(X[indecies], y[indecies], W, reg=reg)
		W = prox(W - lr * W_grad, reg * lr)
		cost = find_cost(X[indecies], y[indecies], W, reg=reg)
...
if __name__ == '__main__':
	...
	five_columns_indecies = np.argsort(-W[:, 0])[:5] + 1
	top_features = np.array(df.columns)[five_columns_indecies]
	"""*** END YOUR CODE HERE ***"""
	print(top_features)
