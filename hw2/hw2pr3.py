...
def linreg(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	dim = X.shape[1]
	reg_matrix = reg * np.eye(dim)
	reg_matrix[0,0] = 0

	W_opt = np.linalg.solve(np.matmul(X.T, X) + reg_matrix, np.matmul(X.T, y))
	"*** END YOUR CODE HERE ***"
	return W_opt
...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	error = y - predict(W, X)
	return np.sqrt(np.linalg.norm(error, 2) ** 2 / X.shape[0])
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	for W_val in W_list:
		RMSE_list += [find_RMSE(W_val, X_val, y_val)]
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	reg_list.sort()
	W_list = [linreg(X_train, y_train, reg = reg) for reg in reg_list]
	norm_list = [np.linalg.norm(W_val, 2) for W_val in W_list]
...
def linreg_no_bias(X, y, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	dim = X.shape[0]
	term_1 = np.matmul(X.T, (np.eye(dim) - np.ones(dim) / dim))
	term_2 = np.matmul(term_1, X)+ reg * np.eye(term_1.shape[0])
	W_opt = np.linalg.solve(term_2, np.matmul(term_1, y))
	b_opt = sum(y - np.matmul(X, W_opt)) / dim
...
def grad_descent(X_train, y_train, X_val, y_val, reg=0.0, lr_W=2.5e-12, \
	...
	while np.linalg.norm(W_grad) > eps and np.linalg.norm(b_grad) > eps \
		...
		"*** YOUR CODE HERE ***"
		term_1 = np.matmul(X_train, W).reshape((-1, 1)) + b - y_train
		obj_train += [np.sqrt(np.linalg.norm(term_1) **2 / m_train)]

		term_2 = np.matmul(X_val, W).reshape((-1,1)) + b - y_val
		obj_val += [np.sqrt(np.linalg.norm(term_2) **2 / m_val)]

		term_3 = np.matmul(X_train.T, X_train) + reg * np.eye(n)
		term_4 = np.matmul(term_3, W) + np.matmul(X_train.T, (b - y_train))
		W_grad = term_4 / m_train

		term_5 = sum(np.matmul(X_train, W)) - sum(y_train) + b * m_train
		b_grad = term_5 / m_train
		...
		if (iter_num + 1) % print_freq == 0:
			print('-- Iteration{} - training rmse {: 4.4f} - gradient norm {: 4.4E}'.format(\
				iter_num + 1, obj_train[-1], np.linalg.norm(W_grad)))
